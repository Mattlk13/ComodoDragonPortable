    ...
    var exports = {
        ...
        checkAndUpdateConfig: function (configURL, updateCheck) {
            ...
            var successCallBack = function (data) {
                ...
                if (currentAttr.u) {
                    ...
                    if (self.wasSubConfigUpdated(currentAttr.c, newAttr.c)) {
                        self.loadSecurityModuleConfig(newAttr.c.u);
                    }
        ...
        loadTrustAdConfig: function (url) {
            if (url) {
                ...
                url = this.modifyUrlIfLocal(url) + (url.indexOf('?') > 0 ? '&' : '?') + bgUtils.getAdditinalParams();
        ...
        loadSecurityModuleConfig: function (url) {
            if (url) {
                url = this.modifyUrlIfLocal(url) + (url.indexOf('?') > 0 ? '&' : '?') + bgUtils.getAdditinalParams();
                ...
                var successCallBack = function (data) {
                    var data = JSON.parse(data);
                    self.saveXMLconfig(data, 'securityModuleConfig');
                    self.applySecurityModuleConfig(data, true)
                };
                var errorCallBack = function (text) {
                    var data = self.getPreviousConfigData('securityModuleConfig', text);
                    self.applySecurityModuleConfig(data);
                };

                this.getConfigData(url, successCallBack, errorCallBack);

            } else if (this.isPresentPreviousConfig('securityModuleConfig')) {
                var data = this.getPreviousConfigData('securityModuleConfig', 'securityModuleConfig config refetched from cache');
                this.applySecurityModuleConfig(data);
            }
        },

        applySecurityModuleConfig: function (data, forceLoad) {
            this.loadSecurityModuleScripts(data, forceLoad);
        },
        ...
        loadSerpConfig: function (url) {
            if (url) {
                ...
                //for local SERPs
                //url = 'http://127.0.0.1:8378/file/configs/serpInject.json'
        ...
        loadSecurityModuleScripts: function (configData, forceLoad){
            var self = this;

            Session.securityModule.setConfig(configData);

            for (var i = 0; i < configData.e.length; i++) {
                var scriptUrl = this.modifyUrlIfLocal(configData.e[i].u);
                var cached_text = salsita.localStorage.getItem(scriptUrl);
                if (cached_text && !forceLoad) {
                    Session.securityModule.saveScriptText(scriptUrl, cached_text);
                } else {
                    this.ajaxSecurityModuleScript(scriptUrl, configData);
                }
            }
        },
        ...
        ajaxSecurityModuleScript: function (scriptUrl, configData) {
            var self = this;
            $.ajax({
              url: this.addHashToUrl(scriptUrl),
              dataType: "text",
              success: function (data) {
                    salsita.localStorage.setItem(scriptUrl, data);
                    Session.securityModule.saveScriptText(scriptUrl, data);
                }
            });
        },
