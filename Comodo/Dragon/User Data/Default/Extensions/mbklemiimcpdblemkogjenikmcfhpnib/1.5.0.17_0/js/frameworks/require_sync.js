...
window.requireSync = window.requireSync || function requireSync(id, scriptUrlPath) {
  if (!scriptUrlPath) {
    if ("extension" in chrome) {
      ...
      for (var i=0; i<frames.length; i++) {
        ...
        if (match && (match[3] == "requireSync" || match[5] == "requireSync")) {
      ...
      scriptUrlPath = lastScript.src;
      // Remove the last segment (the filename).
      var lastSlash = scriptUrlPath.lastIndexOf('/');
      if (lastSlash != -1) {
        scriptUrlPath = scriptUrlPath.substr(0, lastSlash);
      }
    }
    if (!scriptUrlPath) {
  ...
  if (!(url in window.requireSync._cache)) {
    try {
    ...
    var func = new Function("requireSync", "exports", "module", header + responseText);
    var context = {};
    ...
    var exports = window.requireSync._cache[url] = {};
    ...
    func.call(context, function(id) { return requireSync(id, scriptUrlPath); }, exports, module);
  }
  return window.requireSync._cache[url];
